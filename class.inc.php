...
class Form{
	...
	public function zgloszenieForm($method, $action, $idzadania)
	{
		...
		echo '<tr>';
		echo '<td>Treść zgłaszanej uwagi/błędu</td>';
		echo '<td><textarea name=tresc rows=5 cols=40></textarea></td>';
		echo '</tr>';
		echo '<tr>';
		echo '<td>Imię, nazwisko lub nick</td>';
		...
		echo '<tr>';
		echo '<td>&nbsp;</td>';
		echo '<td><input type= submit name= submit  value=Wyślij >&nbsp';
		echo '<input type=reset value=Wyczyść></td></form>';
		echo '</tr>';
		echo '</table>';
		echo '<a href="index.php"> Cofnij do index </a>';
	} 
  ...
  echo $e->getMessage(); //Boring error messages from anything else!
	...
	public function show_all()
	{
		...
		foreach($sql as $row)
		{
			...
			{ 
				echo '<div><a href="?showzad='.$row['id_zadanie'].'">Zadanie numer:'.$row['id_zadanie'].'</a> | <a href="?editzadform='.$row['id_zadanie'].'">Edytuj</a> | <a href="?zgloszenieform='.$row['id_zadanie'].'">Zgłoś uwagę/błąd</a>';
				echo '<br></div>';
			}        
		}
		$sql->closeCursor();
